    ...
    getBlockByHeight() {
        this.app.get("/block/:height", async (req, res) => {
            if(req.params.height) {
                try {
                    if (req.params.height.length == 64) {
                        const hash = req.params.height;
                        var block = await this.blockchain.getBlockByHash(hash);

                    } else {
                        const height = parseInt(req.params.height);
                        var block = await this.blockchain.getBlockByHeight(height);
                    }
